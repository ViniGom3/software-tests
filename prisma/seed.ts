...
const alunos = Array.from({ length }).map((_, i) => ({
  cursoId: i,
}));
const curso = Array.from({ length }).map((_, i) => ({
  nome: 'Curso ' + i,
  codigo: i,
}));

const turma = Array.from({ length: 2 }).map((_, i) => ({
...
const avaliacao = Array.from({ length: 200 }).map((_, i) => ({
